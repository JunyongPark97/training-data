...
class OCRSearchRequestSourceForm(forms.ModelForm):
...
class OCRSearchRequestBoxInline(admin.TabularInline):
    model = OCRSearchRequestBox
    fields = ['get_image', 'mathpix_latex', 'latex']
    readonly_fields = ['get_image','mathpix_latex']
...
class OCRSearchRequestSourceAdmin(admin.ModelAdmin):
    ...
    change_form_template = 'source/admin/source_change_form.html'
    inlines = [OCRSearchRequestBoxInline, ]
    ...
    def get_changelist_form(self, request, **kwargs):
        return OCRSearchRequestSourceForm
    ...
    def import_csv(self, request):
        if request.method == 'POST':
            ...
            new_testsets = []
            for line in reader:
                new_testset = OCRSearchRequestSource(
            ...
            OCRSearchRequestSource.objects.bulk_create(new_testsets)
...
class OCRSearchRequestBoxAdmin(admin.ModelAdmin):
    list_display = ['id', 'source', 'get_image_url', 'mathpix_latex', 'latex', 'valid']
    ...
    mark_as_valid.short_description = "Mark Selected OCRSearchRequestBoxes Valid"
...
admin.site.register(OCRSearchRequestBox, OCRSearchRequestBoxAdmin)
