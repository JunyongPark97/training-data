...
class Source(models.Model):
    ...
    @property
    def prev(self):
        return Source.objects.filter(pk__lt=self.pk).order_by('pk').last()

    @property
    def next(self):
        return Source.objects.filter(pk__gt=self.pk).order_by('pk').first()


class Box(models.Model):
    source = models.ForeignKey(Source, related_name='boxes', on_delete=models.CASCADE)
    ...
    label = models.IntegerField(choices=((0, 'word'), (1, 'picture'), (2, 'graph') ))
